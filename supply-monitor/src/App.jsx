...
const App = () => {
  ...
  // Controle de Drill-down (Detalhes da Barra)
  ...
  const handleDownloadExcel = (dataSet, sheetName) => {
    ...
    const exportData = dataSet.map(item => ({
      PEDIDO: item.PEDIDO || item.PI || "S/N",
      PI: item.PI || "-",
      STATUS: item.STATUS,
      DATA_ENTRADA: item.entryDateIso ? new Date(item.entryDateIso).toLocaleDateString('pt-BR') : "-",
      DIAS_EM_ABERTO: item.daysOpen
    }));
